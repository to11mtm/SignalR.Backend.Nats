...
internal class NatsProtocol
{
    private readonly DefaultHubMessageSerializer _messageSerializer;

    public NatsProtocol(DefaultHubMessageSerializer messageSerializer)
    {
        _messageSerializer = messageSerializer;
    }

    // Adaptation of the Redis Protocol for SignalR
    // https://github.com/aspnet/SignalR/blob/dev/specs/RedisProtocol.md
    // The Redis Protocol:
    ...
    public static byte[] WriteGroupCommand(NatsGroupCommand command)
    {
        // Written as a MessagePack 'arr' containing at least these items:
    ...
    public static NatsInvocation ReadInvocation(ReadOnlyMemory<byte> data)
    {
        ...
        return new NatsInvocation(message, excludedConnectionIds, invocationId, returnChannel);
    }

    public static NatsGroupCommand ReadGroupCommand(ReadOnlyMemory<byte> data)
    {
        var reader = new MessagePackReader(data);
        ...
        return new NatsGroupCommand(id, serverName, action, groupName, connectionId);
    }
    ...
    public static NatsCompletion ReadCompletion(ReadOnlyMemory<byte> data)
    {
        ...
        return new NatsCompletion(protocolName, ros ?? new ReadOnlySequence<byte>());
    }
